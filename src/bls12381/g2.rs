...
impl G2Affine {
    ...
    fn to_compressed(&self) -> [u8; 96] {
        ...
        let x = Fq2::conditional_select(&self.x, &Fq2::zero(), self.is_identity());
    ...
    fn to_uncompressed(&self) -> [u8; 192] {
        ...
        let x = Fq2::conditional_select(&self.x, &Fq2::zero(), self.is_identity());
        let y = Fq2::conditional_select(&self.y, &Fq2::zero(), self.is_identity());
    ...
    /// for details about how group elements are serialized.
    fn from_uncompressed(bytes: &[u8; 192]) -> CtOption<Self> {
    ...
    fn from_uncompressed_unchecked(bytes: &[u8; 192]) -> CtOption<Self> {
    ...
    /// for details about how group elements are serialized.
    fn from_compressed(bytes: &[u8; 96]) -> CtOption<Self> {
    ...
    fn from_compressed_unchecked(bytes: &[u8; 96]) -> CtOption<Self> {
        // Obtain the three flags from the start of the byte sequence
        let compression_flag_set = Choice::from((bytes[0] >> 7) & 1);
        let infinity_flag_set = Choice::from((bytes[0] >> 6) & 1);
        let sort_flag_set = Choice::from((bytes[0] >> 5) & 1);
...
impl core::fmt::Debug for G2Compressed {
...
impl core::fmt::Debug for G2Uncompressed {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        self.0[..].fmt(f)
    }
}
